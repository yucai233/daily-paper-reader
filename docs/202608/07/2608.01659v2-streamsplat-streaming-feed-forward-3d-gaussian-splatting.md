...
tags: ["query:q1"]
score: 7.0
evidence: 用于前馈3D渲染的体素对齐场景状态
tldr: 本研究针对现有前馈3D高斯泼溅（3DGS）方法无法处理在线序列视图的问题，提出了StreamSplat框架。该框架通过体素对齐因果缓存（VACC）增量维护场景状态，并引入历史投影深度锚定（HPDA）和缓存引导特征注入（CGFI）来有效利用历史信息。StreamSplat在处理长序列输入时表现优异，解决了内存溢出问题，并实现了随观测增加而持续提升的渲染质量。
selection_source: fresh_fetch
motivation: 现有的前馈3DGS方法通常假设固定视图集且需联合处理，难以应用于需要因果更新和处理长序列的在线流式场景。
method: 提出了一种基于体素缓存的流式框架，利用几何感知的缓存机制和深度锚定技术，实现对长序列视图的增量式场景重建与更新。
result: 在多个数据集上表现与SOTA相当，且能成功处理多达1024帧的长序列，而传统方法在此时会因内存不足而崩溃。
conclusion: StreamSplat证明了通过因果缓存机制可以实现高效且可扩展的流式3D场景重建，为在线实时应用提供了新的技术路径。
---

## 摘要
前馈 3D 高斯泼溅（3D Gaussian Splatting）无需针对每个场景进行优化即可实现高效的新视角合成，但大多数现有方法假设上下文视图集是固定的并对其进行联合处理。这限制了它们在在线场景中的适用性，在这些场景中，校准后的视图按顺序到达，且场景必须以因果方式更新。我们提出了 StreamSplat，这是一个流式前馈 3DGS 框架，它增量地维护一个持久的、基于几何的场景状态，并在每个输入块之后将其解码为可渲染的 3D 高斯。StreamSplat 的核心是体素对齐因果缓存（Voxel-Aligned Causal Cache, VACC），它将历史 3D token 存储在内存受限的体素结构中，使得内存随探索的场景几何形状而非流长度增长。为了在因果预测期间更好地重用历史信息，我们引入了历史投影深度锚定（History-Projected Depth Anchoring, HPDA），将缓存的几何投影为当前代价体积估计的深度引导；以及缓存引导特征注入（Cache-Guided Feature Injection, CGFI），将缓存的潜在证据注入到高斯 token 回归中。在 DL3DV、RealEstate10K 和 ScanNet 上的实验表明，尽管不使用未来视图或全场景上下文，StreamSplat 在稀疏因果输入下仍能与最先进的前馈 3DGS 方法保持竞争力。更重要的是，它可以扩展到包含 256、512 和 1024 个视图的长输入流，而固定视图基准在这些情况下会出现内存溢出，随着更多观测数据的到达，新视角合成质量持续提升。代码将在论文被接收后公开。
