...
tldr: 本研究提出SLARM，一种用于动态场景重建的前馈模型，集成了语义理解与实时流式推理。该模型通过高阶运动建模捕捉复杂非均匀运动，并利用LSeg蒸馏实现语言对齐的语义表示，支持自然语言查询。SLARM采用窗口因果注意力机制，实现了低延迟、无内存累积的流式处理。实验表明，该统一框架在动态估计、渲染质量和场景解析方面均达到领先水平，显著提升了运动精度和重建效果。
selection_source: fresh_fetch
motivation: 旨在解决动态场景重建中复杂运动捕捉难、缺乏语义理解以及无法进行实时流式推理的问题。
method: 结合高阶运动建模、LSeg语义特征蒸馏以及基于窗口的因果注意力机制，构建了一个统一的前馈推理框架。
result: "在动态估计精度上提升了21%，重建PSNR提升了1.6 dB，且场景分割mIoU相比现有方法提高了20%。"
conclusion: SLARM通过紧密耦合几何与语义信息，实现了高效、稳健且支持自然语言交互的动态场景实时重建。
---
