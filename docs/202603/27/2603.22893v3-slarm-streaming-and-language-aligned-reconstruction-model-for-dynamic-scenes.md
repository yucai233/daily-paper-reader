...
tags: ["query:q1"]
score: 6.0
evidence: 基于可微分渲染训练的动态场景重建
tldr: 本文提出SLARM，一种用于动态场景重建的前馈模型，集成了语义理解与实时流式推理。该模型通过高阶运动建模捕捉复杂非均匀运动，并利用LSeg蒸馏实现语言对齐的语义表示，支持自然语言查询。此外，采用窗口因果注意力机制实现了低延迟、内存稳定的流式处理。SLARM在动态估计、渲染质量和场景分割上均达到SOTA水平，显著提升了重建的准确性与鲁棒性。
selection_source: fresh_fetch
motivation: 旨在解决动态场景重建中复杂运动捕捉难、缺乏语义理解以及无法进行低延迟流式推理的问题。
method: 结合高阶运动建模、LSeg语义特征蒸馏以及基于窗口的因果注意力机制，构建了一个统一的前馈推理框架。
result: "相比现有方法，运动精度提升了21%，重建PSNR提升了1.6 dB，场景分割mIoU提升了20%。"
conclusion: SLARM成功统一了动态几何重建与语言对齐的语义理解，为实时、可交互的动态场景分析提供了高效方案。
---
