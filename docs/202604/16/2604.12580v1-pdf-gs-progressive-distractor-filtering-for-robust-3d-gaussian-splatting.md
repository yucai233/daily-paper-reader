...
title_zh: PDF-GS：鲁棒 3D 高斯泼溅的渐进式干扰物过滤
...
tldr: 针对3D高斯泼溅（3DGS）在处理多视图不一致（如动态干扰物）时易产生伪影的问题，本文提出PDF-GS框架。该方法利用3DGS固有的信号抑制能力，通过渐进式多阶段优化策略，利用差异线索逐步过滤干扰物并恢复一致的精细细节。PDF-GS在多个数据集上达到SOTA性能，且无需修改架构，具有轻量级和易适配的特点。
selection_source: fresh_fetch
motivation: 传统的3DGS训练假设多视图完全一致，导致其在面对现实场景中的干扰物时容易产生视觉伪影。
method: 提出一种渐进式干扰过滤框架，通过多阶段优化利用差异线索剔除不一致信号，并结合重建阶段恢复高质量的视图一致细节。
result: 在多种挑战性真实场景下，PDF-GS在保持轻量级的同时显著超越了现有基准方法，实现了鲁棒且无干扰的高保真重建。
conclusion: PDF-GS通过挖掘3DGS的自过滤潜力，为鲁棒的三维场景重建提供了一种无需额外推理开销且易于集成的有效解决方案。
---

## 摘要
3D 高斯泼溅（3DGS）的最新进展实现了令人印象深刻的实时照片级真实感渲染。然而，传统的训练流程本质上假设输入图像之间具有完全的多视图一致性，这使得它们对违反该假设并导致视觉伪影的干扰物（distractors）非常敏感。在这项工作中，我们重新审视了 3DGS 一个尚未被充分探索的方面：其抑制不一致信号的固有能力。基于这一见解，我们提出了 PDF-GS（Progressive Distractor Filtering for Robust 3D Gaussian Splatting），这是一个通过渐进式多阶段优化来增强这种自过滤特性的框架。渐进式过滤阶段通过利用差异线索逐渐移除干扰物，而随后的重建阶段则从纯化后的高斯表示中恢复细粒度的、视图一致的细节。通过这种迭代优化，PDF-GS 实现了鲁棒、高保真且无干扰物的重建，在各种数据集和具有挑战性的现实条件下始终优于基准方法。此外，我们的方法是轻量级的，且易于适配现有的 3DGS 框架，无需更改架构或增加额外的推理开销，从而达到了新的最先进性能。代码已在 https://github.com/kangrnin/PDF-GS 公开。
