...
tags: ["query:q3"]
score: 6.0
evidence: 多视图视觉中的射线级相对位置编码
tldr: 针对多视角视觉Transformer在相机异构（如不同视场角或投影模型）下位置编码失效的问题，本文提出了G-ray。该方法通过将旋转位置编码参数化为相机局部射线角度，实现了跨投影的几何一致性。G-ray无需额外学习参数，在3D重建和新视角合成任务中表现优异，且具备极强的跨投影泛化能力，即使在仅训练于针孔相机的情况下也能处理非针孔输入。
selection_source: fresh_fetch
motivation: 现有基于图像平面的相对位置编码在处理异构相机时会产生投影依赖的相位偏差，导致几何线索不一致。
method: 提出G-ray射线级相对位置编码，利用相机局部射线角度参数化旋转相位，确保相同射线对在不同投影下产生一致的相对相位。
result: "在多视角3D重建基准测试中，G-ray在六项指标上均领先，并将点图相对误差降低了45.8%，同时在新视角合成任务中也展现了显著性能提升。"
conclusion: G-ray通过提供投影不变的几何位置编码，有效解决了多视角视觉Transformer在相机异构场景下的几何一致性难题。
---
